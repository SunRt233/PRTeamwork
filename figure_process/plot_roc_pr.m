basepath = 'E:/Project/python/PatternRecognitionTeamwork/saves/';
% 定义所有要读取的文件路径模式
...
for clfIdx = 1:length(uniqueClassifiers)
    ...
    for roundIdx = 1:length(uniqueRounds)
        ...
        fig = figure('Name', sprintf('%s, Round = %.0f', classifierType, roundNumber), 'NumberTitle', 'off');
        ...
        hold off;
         ...
         % 检查并创建保存目录
        saveDir = strcat(basepath,'figures');
        ...
        try
            ...
            warning('Failed to save the SVG file: %s', ME.message);
        end
    end
end